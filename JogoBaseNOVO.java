...
class JogoBaseNOVO extends JFrame {
    //// DADOS PLAYER
    ...
    boolean boolGameOver = false, boolLastBombaBlock = false;
    ...
    Timer refreshModels = new Timer(25, e -> {
        ...
        try {
            ...
            if (boolLastBombaBlock) { // Rotina para checar as bombas recém colocadas e quando elas irão bloquear o player
                if (!arrayBombas.isEmpty()) {
                    ...
                    if (!new Rectangle(player1.X, player1.Y + 15, 35, 40).intersects(arrayBombas.get(arrayBombas.size() - 1).getHitBox())) {
                        arrayBombas.get(arrayBombas.size() - 1).boolBloqueando = true;
                        boolLastBombaBlock = false;
                    }
                }
            }
        ...
        Fase(int numeroFase){
            try {
                if (numeroFase == MULTIPLAYER) {
                    ...
                    try {
                        imagensAmbiente[FUNDO] = new ImageIcon("Resources/FaseMultiplayer/chao1.png").getImage();
    ...
    boolean intersBombas(Rectangle checkR){
        for (Bomba bombaBlock : arrayBombas) {
            if (bombaBlock.boolBloqueando && checkR.intersects(bombaBlock.getHitBox()))
                return false;
        }
        return true;
    }
    ...
    boolean intersBombasInimigos(Rectangle checkR){
        for (Bomba bombaBlock : arrayBombas) {
            if (checkR.intersects(bombaBlock.getHitBox()))
                return false;
        }
        return true;
    }
        ...
        void move(){
            ...
            if (sentido == HORIZONTAL) {
                ...
                if (x <= 850 && !inverteMovimento && inimigoIntersX(new Rectangle(x + velocidade, y, 40, 40), fase.arrayBlocosQuebraveis) && intersBombasInimigos(new Rectangle(x + velocidade, y, 40, 40))) {
                    ...
                    x += velocidade;
                } else if((!intersBombasInimigos(new Rectangle(x, y, 40, 40)))) {
                    inimigo = esquerda.getImage();
                    while((!intersBombasInimigos(new Rectangle(x, y, 40, 40)))) {
                        x += velocidade;
                    }
    ...
    class layoutDeCima extends JPanel {
        ...
        Image layoutBackGround = new ImageIcon("Resources/Layout/fundoBarraSuperior.png").getImage();
        Timer tempo;
